...
**強化学習は不要**と確認済み: 同じ情報・同じサイジングで比較すると 4 fold 平均で
RL +0.14 対 ルール +0.67（RL が勝ったのは 1/4 fold）。シグナルが 1 本の特徴量の
単調関数である以上、ルールがそれを捉えきり、RL は推定誤差と分散を足すだけになる。

**GMO 実装は 5 銘柄（BTC/ETH/XRP/LTC/BCH）で期待値 Sharpe +0.92 前後**
（7 銘柄 +0.99、3 銘柄 +0.78）。設定は `configs/gmo_live.yaml`。
