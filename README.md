...
| 検証した時代（いずれもホールドアウトではない） | Sharpe | 年率 | 最大DD | Calmar |
|---|---:|---:|---:|---:|
| 時代A 2017-10〜2020-05 / Huobi 6 銘柄 | **1.64** | +31.7% | −10.8% | 2.92 |
| 時代B 2020-01〜2024-12 / Perp 7 銘柄 | **1.82** | +35.3% | −12.4% | 2.84 |
| 時代B / GMO 相当 3 銘柄（BTC・ETH・XRP） | 1.56 | +29.6% | −15.0% | 1.98 |

改良前（ロングのみ・14 日単独）は 時代A 0.94 / 時代B 1.59・最大DD −27.5% だった。
**最大の発見は「単一銘柄では損なショートが、分散すると下落局面の収益源になる」**こと
（2018 年 −0.13→**+1.74**、2022 年 +0.52→**+1.09**）。

収益源の多様化も試したが、**トレンド以外に機能する収益源は見つからなかった**
（短期リバーサル −1.55/−2.02、CS リバーサル −0.85/−3.40、ファンディング／ベーシス／OI も上乗せ無し）。
分散は「収益源の分散」ではなく「銘柄の分散」で取る設計になっている。

⚠️ **ホールドアウト（2025-01〜2026-03）は改良版では未使用**。最後の一回として温存している。
旧版（ロングのみ）でのホールドアウト実績は Sharpe +0.61 / +20.5%（同期間の買い持ちは −19.2%）。
